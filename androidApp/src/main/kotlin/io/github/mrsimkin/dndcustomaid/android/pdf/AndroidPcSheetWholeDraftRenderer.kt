// GENERATED by scripts/generate_android_pc_sheet_renderer.py from DesktopPcSheetWholeDraftRenderer.kt.
 ...
 */
internal class AndroidPcSheetWholeDraftRenderer(
    ...
    fun renderDraft(
        ...
        if (plan.snapshot.spellbook != null || portraitBytes != null) {
            ...
            PDDocument.load(baseBytes).use { document ->
                portraitBytes?.let { bytes ->
                    AndroidPortraitRenderer(document).overlay(plan, bytes)
                }
                if (plan.snapshot.spellbook != null) {
                    AndroidSpellbookRenderer(document).append(plan)
                }
        ...
        PDDocument.load(familyBytes).use { document ->
            AndroidPcSheetVisualCompletionRenderer(
                document = document,
    ...
    private fun renderFamilyDraft(
        ...
        val mainProofBytes = ByteArrayOutputStream().use { buffer ->
            AndroidPcSheetTemplateProofRenderer(resourceLoader).renderMainPage(plan, buffer)
            buffer.toByteArray()
        }

        PDDocument.load(mainProofBytes).use { mainProof ->
            PDDocument.load(templateBytes).use { sourceTemplate ->
                PDDocument().use { draft ->
                    draft.importPage(mainProof.getPage(0))
                    val sharedBase = AndroidCustomV2SharedBaseRenderer(draft, resourceLoader)
                    ...
                    AndroidCustomV2ExtendedRenderer(
                        document = draft,
    ...
    private fun renderCustomV1HybridDraft(
        ...
        PDDocument.load(templateBytes).use { sourceTemplate ->
            PDDocument().use { draft ->
                val hybrid = AndroidCustomV1HybridRenderer(draft, resourceLoader)
                val primitives = AndroidPdfRenderingPrimitives(
                    AndroidPdfFontRegistry(draft, resourceLoader = resourceLoader),
                )
                ...
                AndroidCustomV1ExtendedRenderer(
                    document = draft,
                    sourceTemplate = sourceTemplate,
                    resourceLoader = resourceLoader,
                ).appendExtendedPages(plan)
    ...
    private fun drawNotesDoodles(
        ...
        stream.setStrokingColor(com.tom_roush.harmony.awt.AWTColor.BLACK)
