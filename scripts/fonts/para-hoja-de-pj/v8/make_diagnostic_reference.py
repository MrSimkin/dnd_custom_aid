...
FONT='/mnt/data/Para_Hoja_de_PJ_Symbols_v8.ttf'
OUT='/mnt/data/Para_Hoja_de_PJ_v8_diagnostic_reference.pdf'
pdfmetrics.registerFont(TTFont('PJv8', FONT))
W,H=landscape(A4); M=14*mm
c=canvas.Canvas(OUT,pagesize=(W,H), invariant=1); c.setTitle('Para Hoja de PJ Symbols v8 - Diagnostic and Reference'); c.setAuthor('Gustavo Munoz / DnD Custom Aid')
page=0
...
def header(title,subtitle=''):
    ...
    c.setFont('Helvetica',8); c.drawRightString(W-M,8*mm,f'v8 APPROVED / FROZEN - page {page}')

def glyph(ch,x,y,size):
    c.setFillColor(colors.black); c.setFont('PJv8',size)
...
# 1 contract
header('Para Hoja de PJ Symbols v8 - diagnostic + reference', "APPROVED / FROZEN. Owner pre-authorized approval once 6 and 9 were corrected; v8 changes only those two glyphs from v7 and passes technical/visual QA.")
lines=[
...
'7. Container marks are SHAPE-AWARE full interior spans; endpoints follow each real inner contour.',
'8. v8 changes ONLY digits 6 and 9 from v7; all other glyphs and mappings remain unchanged.',
...
# ordinary characters
header('Ordinary keyboard characters', 'Digits and common punctuation are real glyphs in the font. v8 changes only 6/9, using lighter ordinary ring-and-sweep forms that match the accepted numeral family.')
...
header('Ordinary numerals 0-9 - large visual inspection', 'v8 final numeral QA: only 6 and 9 are redrawn from v7; verify their lighter monoline weight, clean bowl/sweep join, and consistency at large and sheet sizes.')
num_y=H/2+25*mm
...
for size,yy in [(32,20*mm),(22,12*mm)]:
    ...
    c.setFont('PJv8',size); c.drawString(62*mm,yy,'0123456789  -12  +3  20/20  100%')
end()

# dedicated full-span mark interpretation QA
header('Full-span internal mark semantics - large inspection', "v8 preserves v7's approved shape-aware mark system: each stroke endpoint derives from the container's actual fillable inner contour: full-looking spans with no outline overlap, white wedges, or arbitrary short lines.")
