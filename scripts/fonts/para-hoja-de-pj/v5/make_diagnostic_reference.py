...
FONT='/mnt/data/Para_Hoja_de_PJ_Symbols_v5_final.ttf'
OUT='/mnt/data/Para_Hoja_de_PJ_v5_final_diagnostic_reference.pdf'
pdfmetrics.registerFont(TTFont('PJv5', FONT))
W,H=landscape(A4); M=14*mm
c=canvas.Canvas(OUT,pagesize=(W,H), invariant=1); c.setTitle('Para Hoja de PJ Symbols v5 - Diagnostic and Reference'); c.setAuthor('Gustavo Munoz / DnD Custom Aid')
page=0
...
def header(title,subtitle=''):
    ...
    c.setFont('Helvetica',8); c.drawRightString(W-M,8*mm,f'v5 candidate - page {page}')

def glyph(ch,x,y,size):
    c.setFillColor(colors.black); c.setFont('PJv5',size)
...
# 1 contract
header('Para Hoja de PJ Symbols v5 - diagnostic + reference', 'Candidate for owner visual review. v4 remains preserved as a rejected visual candidate; v5 is the corrected successor.')
lines=[
...
'6. Single check is now a conventional checkmark; double check is two normal checks STACKED vertically.',
'7. Fillable/container shapes have precomposed /, backslash, X, *, +, -, check and dot variants.',
'8. Ordinary digits 0-9 and common punctuation are present as normal keyboard characters.',
...
# ordinary characters
header('Ordinary keyboard characters', 'Digits and common punctuation are real glyphs in the font, not specimen labels from Helvetica.')
...
def marked_pages(title_prefix, base, shapes, generic=False):
    ...
    for part in range((len(shapes)+per_page-1)//per_page):
        ...
        header(f'{title_prefix} ({part+1}/{(len(shapes)+per_page-1)//per_page})', f'Precomposed marked states. Columns: /, backslash, X, *, +, -, check, dot. Namespace: {namespace}.')
