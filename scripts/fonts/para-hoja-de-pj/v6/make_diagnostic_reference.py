...
FONT='/mnt/data/Para_Hoja_de_PJ_Symbols_v6.ttf'
OUT='/mnt/data/Para_Hoja_de_PJ_v6_diagnostic_reference.pdf'
pdfmetrics.registerFont(TTFont('PJv6', FONT))
W,H=landscape(A4); M=14*mm
c=canvas.Canvas(OUT,pagesize=(W,H), invariant=1); c.setTitle('Para Hoja de PJ Symbols v6 - Diagnostic and Reference'); c.setAuthor('Gustavo Munoz / DnD Custom Aid')
page=0
...
def header(title,subtitle=''):
    ...
    c.setFont('Helvetica',8); c.drawRightString(W-M,8*mm,f'v6 candidate - page {page}')

def glyph(ch,x,y,size):
    c.setFillColor(colors.black); c.setFont('PJv6',size)
...
# 1 contract
header('Para Hoja de PJ Symbols v6 - diagnostic + reference', 'Candidate for owner visual review. v5 remains preserved as the almost-final prior candidate; v6 corrects mark-span, numeral, and check geometry.')
lines=[
...
'6. Single check is a clean conventional stroke-based checkmark; double check is two copies STACKED vertically.',
'7. Container marks are FULL-SPAN interior strokes: /, backslash, X, *, +, -, check and dot.',
'8. Ordinary digits 0-9 were redesigned as normal rounded/sans numerals; common punctuation is present from the keyboard.',
...
# ordinary characters
header('Ordinary keyboard characters', 'Digits and common punctuation are real glyphs in the font. Numerals are the redesigned rounded/sans v6 set, not seven-segment figures.')
...
header('Ordinary numerals 0-9 - large visual inspection', 'Redesigned v6 figures: ordinary rounded/sans forms, consistent weight, no seven-segment display construction.')
num_y=H/2+25*mm
...
for size,yy in [(32,20*mm),(22,12*mm)]:
    ...
    c.setFont('PJv6',size); c.drawString(62*mm,yy,'0123456789  -12  +3  20/20  100%')
end()

# dedicated full-span mark interpretation QA
header('Full-span internal mark semantics - large inspection', 'These are lines/symbols drawn ACROSS the usable interior of the container, not tiny punctuation placed in the middle.')
