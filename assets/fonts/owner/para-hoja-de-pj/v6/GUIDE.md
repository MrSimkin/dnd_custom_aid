# Para Hoja de PJ Symbols v6 - guide and renderer reference

Status: **v6 final candidate; deterministic build validated; owner final visual approval pending**.

Author/owner: **Gustavo Muñoz**.  
Technical reconstruction/versioning assistance: OpenAI for DnD Custom Aid.

v6 supersedes the almost-final v5 candidate while preserving v5 unchanged. v1 remains the immutable archival original; v4 and v5 remain preserved historical candidates.

The owner specifically rejected three details in v5: the internal marks were too small and read like punctuation instead of marks drawn across a form field; the numerals were too display-like; and the checkmark still showed awkward geometry. v6 addresses exactly those three issues while preserving the broader mapping and symbol inventory.

## Design contract implemented in v6

v6 retains the four design groups requested by the owner:

1. **All historical v1 concepts A-E/a-e in the v1 design model.** In practice the v6 glyphs for A-E copy the authoritative v1 contours byte-for-byte and preserve the historical metrics.
...
3. **The historical A-E concepts reinterpreted in a v3-derived geometric model.** The actual v3 generator retained A-E, so v6 derives this second model from v3's added circle/check/double-check/cross/diamond geometry and applies it consistently to the historical concepts.
...
Fillable/container shapes also have **precomposed full-span marked states** for `/`, `\`, `X`, `*`, `+`, `-`, `✓`, and dot. The slash/backslash are genuine diagonal strokes running across the usable interior; X combines both directions; + and - span the central axes. These are real glyphs rather than PDF overlay tricks, so the mark/container relationship survives small-size rendering.
...
- v6 file: `v6/Para Hoja de PJ Symbols v6.ttf`;
- v6 SHA-256: `b23cc8ce806ba5e6e6039fc1b24f8457bf91c726d6ecfc43451868d90b2ef02f`;
- deterministic builder SHA-256 at publication: `75db59fa6f2ca2cce9dd8c81d64e1095afa375e82652c6fe298cd540d9033547`;
- diagnostic/reference PDF SHA-256 at publication: `9996a3bee74acc9ddb1c40b9b595213f1ee40c9f3e2b00e91448258cb12b9374`;
- diagnostic/reference source SHA-256 at publication: `3e6c00eef5e696ab0da7d6c81cdcb62e774056f0e4e0283aaf9ffc00780e623f`;
- units per em: `2048`;
- glyph count: `291`;
- cmap mappings: `502`;
- embedding flags: `fsType = 0`;
- family: `Para Hoja de PJ Symbols v6`;
- PostScript name: `ParaHojadePJSymbolsV6-Regular`;
- version: `6.00`, dated `2026-09-18`.

## Historical A-E compatibility

| Historical key | v1/v6 historical meaning | v6 historical implementation | v3-derived equivalent key |
| --- | --- | --- | --- |
...
The numerals were redrawn in v6 as an ordinary rounded/sans set rather than the rejected seven-segment construction. Punctuation/operators remain simple geometric forms. No third-party typeface is imported into the owner-authored font.

| Standard Unicode | v6 glyph |
| --- | --- |
...
Builder: `scripts/fonts/para-hoja-de-pj/v6/build_para_hoja_de_pj_v6.py`

```bash
python scripts/fonts/para-hoja-de-pj/v6/build_para_hoja_de_pj_v6.py \
  assets/fonts/owner/para-hoja-de-pj/v1/Para\ Hj\ De\ Pj.ttf \
  /tmp/Para_Hoja_de_PJ_Symbols_v6.ttf
```

The builder fails closed if the v1 source SHA is not the authoritative archival SHA or if the resulting v6 SHA differs from the frozen deterministic identity.

## Approval boundary

- v1 remains immutable archival provenance;
- v4 remains preserved as the previously published, visually rejected candidate;
- v5 remains preserved unchanged as the almost-final prior candidate, superseded by v6;
- v6 is technically valid, deterministic and feature-complete for this iteration but remains **owner final visual approval pending**;
- the renderer must not switch the production sheet family to v6 until the owner accepts the diagnostic/reference review;
- future changes after a published v6 must create the next derivative rather than silently rewriting v6.
