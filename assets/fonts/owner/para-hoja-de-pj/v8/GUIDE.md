# Para Hoja de PJ Symbols v8 - guide and renderer reference

Status: **v8 APPROVED / FROZEN; deterministic build and visual QA validated**.

Author/owner: **Gustavo Muñoz**.  
Technical reconstruction/versioning assistance: OpenAI for DnD Custom Aid.

v8 supersedes v7 while preserving v7 unchanged. v1 remains the immutable archival original; v4, v5, v6 and v7 remain preserved historical candidates.

The owner accepted v7 except for ordinary numerals `6` and `9`, and explicitly stated that correcting those two would automatically approve the font. v8 changes **only** glyphs `6` and `9` from v7. They are rebuilt as lighter ordinary ring-and-sweep forms whose apparent stroke weight matches the accepted numeral family. Technical, deterministic and PDF visual QA passed, so the stated approval condition is satisfied and v8 is the **approved/frozen font version**.

## Design contract implemented in v8

v8 retains the four design groups requested by the owner:

1. **All historical v1 concepts A-E/a-e in the v1 design model.** In practice the v8 glyphs for A-E copy the authoritative v1 contours byte-for-byte and preserve the historical metrics.
...
3. **The historical A-E concepts reinterpreted in a v3-derived geometric model.** The actual v3 generator retained A-E, so v8 retains this second model from v3's added circle/check/double-check/cross/diamond geometry and applies it consistently to the historical concepts.
...
Fillable/container shapes also have **precomposed shape-aware marked states** for `/`, `\`, `X`, `*`, `+`, `-`, `✓`, and dot. In v8, line endpoints remain derived from each container's actual inner contour rather than from generic scaling. Slash/backslash therefore span the real fillable diagonal, X combines both real diagonals, + and - span the real central axes, and the historical asymmetric oval is handled from its own contour. The strokes stop at the usable interior instead of entering the outline, eliminating the white wedges/overruns seen in v6 while also avoiding arbitrary short lines. These are real glyphs rather than PDF overlay tricks.
...
- v8 file: `v8/Para Hoja de PJ Symbols v8.ttf`;
- v8 SHA-256: `f8f7eeed331be34ea08da23bb165f200c8313e1ba670667466ad8e1aebf77d8b`;
- deterministic builder SHA-256 at publication: `803e11840defbb1b70990c6fca2084091a187a68a1240adec5f269277a5e0eab`;
- diagnostic/reference PDF SHA-256 at publication: `0aea974e8b3a1bd1f674c0369dce68ac31f5cbba0603c5acb39121e1186a8de4`;
- diagnostic/reference source SHA-256 at publication: `5c4d43893c3f27f852ae60ffcda1ba38ed30b03536a5d869b01408a7d880ffd3`;
- units per em: `2048`;
- glyph count: `291`;
- cmap mappings: `502`;
- embedding flags: `fsType = 0`;
- family: `Para Hoja de PJ Symbols v8`;
- PostScript name: `ParaHojadePJSymbolsV8-Regular`;
- version: `8.00`, dated `2026-09-18`.

## Historical A-E compatibility

| Historical key | v1/v8 historical meaning | v8 historical implementation | v3-derived equivalent key |
| --- | --- | --- | --- |
...
The numerals remain an owner-authored ordinary rounded/sans set rather than a seven-segment construction. v8 preserves the accepted v7 forms for `0`, `1`, `2`, `3`, `4`, `5`, `7` and `8`. Only `6` and `9` change: they use a lighter closed bowl plus restrained sweep, with clean joins and an apparent stroke weight aligned to the surrounding numerals. `9` is the coherent rotational partner of `6`. Punctuation/operators remain simple geometric forms. No third-party typeface is imported into the owner-authored font.

| Standard Unicode | v8 glyph |
| --- | --- |
...
`DIAGNOSTIC_REFERENCE.pdf` is the owner visual-QA artifact. It contains: exact-v1 vs v3-derived comparisons, the historical/non-uniform oval vs the additional uniform oval, both core families, corrected conventional single checks and stacked doubles, the approved v8 numeral set (with final 6/9 correction), all shape-aware precomposed marked-container variants in v1/v3/generic ranges, extended icons, standard Unicode aliases, keyboard mappings, generic/style-specific/semantic PUA references, proposed sheet uses, and large/compact stress tests.

The v8 glyph set is owner-approved/frozen under the explicit instruction that correcting 6 and 9 would automatically approve the font; this guide records that approved state after technical and visual QA.

## Deterministic reproduction

Builder: `scripts/fonts/para-hoja-de-pj/v8/build_para_hoja_de_pj_v8.py`

```bash
python scripts/fonts/para-hoja-de-pj/v8/build_para_hoja_de_pj_v8.py \
  assets/fonts/owner/para-hoja-de-pj/v1/Para\ Hj\ De\ Pj.ttf \
  /tmp/Para_Hoja_de_PJ_Symbols_v8.ttf
```

The builder fails closed if the v1 source SHA is not the authoritative archival SHA or if the resulting v8 SHA differs from the frozen deterministic identity.
...
- v6 remains preserved unchanged as an earlier almost-final candidate;
- v7 remains preserved unchanged as the immediate predecessor, superseded only by the final `6/9` correction;
- v8 is **owner approved / frozen**, technically valid, deterministic and feature-complete;
- the font gate is closed: subsequent PC-sheet work may use v8, while any renderer integration/PR merge remains a separate repository change;
- future font changes after approved v8 must create v9 rather than silently rewriting v8.
