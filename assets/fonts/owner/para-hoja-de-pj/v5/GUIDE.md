# Para Hoja de PJ Symbols v5 - guide and renderer reference

Status: **v5 final candidate; deterministic build validated; owner final visual approval pending**.

Author/owner: **Gustavo Muñoz**.  
Technical reconstruction/versioning assistance: OpenAI for DnD Custom Aid.

v5 supersedes the rejected v4 visual direction without rewriting v1 or v4. v1 remains the immutable archival original; v4 remains a preserved published candidate and historical record.

## Design contract implemented in v5

v5 implements the four groups requested by the owner:

1. **All historical v1 concepts A-E/a-e in the v1 design model.** In practice the v5 glyphs for A-E copy the authoritative v1 contours byte-for-byte and preserve the historical metrics.
...
3. **The historical A-E concepts reinterpreted in a v3-derived geometric model.** The actual v3 generator retained A-E, so v5 derives this second model from v3's added circle/check/double-check/cross/diamond geometry and applies it consistently to the historical concepts.
...
The training grammar is fixed as **Competent / Proficient = one conventional check (`✓`)** and **Expertise / Pericia = two copies of that same conventional check stacked vertically**, following the Android app's stacked expertise grammar while correcting the earlier elongated check silhouette.

Fillable/container shapes also have **precomposed marked states** for `/`, `\`, `X`, `*`, `+`, `-`, `✓`, and dot. These are real glyphs rather than PDF overlay tricks, so the mark/container relationship survives small-size rendering.
...
- v5 file: `v5/Para Hoja de PJ Symbols v5.ttf`;
- v5 SHA-256: `5e99861a9e9774c05da253b1bf0963eacd95ae50cef19ac5ffd2ba4a8362b588`;
- deterministic builder SHA-256 at publication: `cf0d20d6421710652b16d8a90fb04eb725884a5f5649f6e0998fbb432d97d757`;
- diagnostic/reference PDF SHA-256 at publication: `891f3a4a1b7dff62e1e3387437ea440542b431b2effb08ce23886042ddc331f8`;
- diagnostic/reference source SHA-256 at publication: `209db9244bc029136930487b7239ce35679992fcd5e461823639c57e9101656e`;
- units per em: `2048`;
- glyph count: `291`;
- cmap mappings: `502`;
- embedding flags: `fsType = 0`;
- family: `Para Hoja de PJ Symbols v5`;
- PostScript name: `ParaHojadePJSymbolsV5-Regular`;
- version: `5.00`, dated `2026-09-18`.

## Historical A-E compatibility

| Historical key | v1/v5 historical meaning | v5 historical implementation | v3-derived equivalent key |
| --- | --- | --- | --- |
...
The numerals/punctuation use a simple geometric technical-sans language so they remain legible without importing a third-party typeface into the owner-authored font.

| Standard Unicode | v5 glyph |
| --- | --- |
...
Marks are encoded in this fixed order inside each container block:

| mark index | mark | intended reading |
| ---: | --- | --- |
| `0` | `/` | slash / partial / one-direction state |
| `1` | `\` | backslash / opposite-direction state |
| `2` | `X` | crossed / unavailable / failed |
| `3` | `*` | special / starred |
| `4` | `+` | added / positive / healed |
| `5` | `-` | reduced / negative / removed |
| `6` | `✓` | checked / completed |
...
Builder: `scripts/fonts/para-hoja-de-pj/v5/build_para_hoja_de_pj_v5.py`

```bash
python scripts/fonts/para-hoja-de-pj/v5/build_para_hoja_de_pj_v5.py \
  assets/fonts/owner/para-hoja-de-pj/v1/Para\ Hj\ De\ Pj.ttf \
  /tmp/Para_Hoja_de_PJ_Symbols_v5.ttf
```

The builder fails closed if the v1 source SHA is not the authoritative archival SHA or if the resulting v5 SHA differs from the frozen deterministic identity.

## Approval boundary

- v1 remains immutable archival provenance;
- v4 remains preserved as the previously published, visually rejected candidate;
- v5 is technically valid, deterministic and feature-complete for this iteration but remains **owner final visual approval pending**;
- the renderer must not switch the production sheet family to v5 until the owner accepts the diagnostic/reference review;
- future changes after a published v5 must create the next derivative rather than silently rewriting v5.