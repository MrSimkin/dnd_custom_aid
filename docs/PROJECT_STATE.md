...
**Current active package:** PC Sheet PDF Export — Classic Run-2 production promotion
...
Resume Wave 7 from `wave7/pc-sheet-pdf-renderer-template-proof` / PR #85 for the active PDF renderer package. Custom-v2 and Custom-v1 production audits are PASS; the next renderer gate is Classic Run-2 production promotion. The semantic/render-plan foundation is integrated; production code must consume it rather than rebuilding export semantics. Routine safe green boundaries do not require separate owner confirmation.
