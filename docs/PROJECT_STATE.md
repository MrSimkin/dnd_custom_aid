...
**Current active package:** PC Sheet PDF Export — portrait-byte handoff + Crop/Fit
...
Resume Wave 7 from `wave7/pc-sheet-pdf-renderer-template-proof` / PR #85 for the active PDF renderer package. Custom-v2, Custom-v1 and Classic integrated production audits are PASS, and the application-owned optional Spellbook production gate is PASS; the next renderer gate is local portrait-byte handoff plus Crop/Fit. The semantic/render-plan foundation is integrated; production code must consume it rather than rebuilding export semantics. Routine safe green boundaries do not require separate owner confirmation.
