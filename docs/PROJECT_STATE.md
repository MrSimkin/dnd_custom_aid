...
**Current active package:** PC Sheet PDF Export — optional application-owned Spellbook
...
Resume Wave 7 from `wave7/pc-sheet-pdf-renderer-template-proof` / PR #85 for the active PDF renderer package. Custom-v2, Custom-v1 and Classic integrated production audits are PASS; the next renderer gate is the optional application-owned Spellbook. The semantic/render-plan foundation is integrated; production code must consume it rather than rebuilding export semantics. Routine safe green boundaries do not require separate owner confirmation.
